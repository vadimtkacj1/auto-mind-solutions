...
function arcOnSphere(R: number, a: [number,number], b: [number,number], steps = 24): THREE.Vector3[] {
  ...
  for (let i = 0; i <= steps; i++) {
    const t = i / steps;
    new THREE.Vector3().copy(va).lerp(vb, t).normalize().multiplyScalar(R * 1.004);
    pts.push(new THREE.Vector3().copy(va).lerp(vb, t).normalize().multiplyScalar(R * 1.004));
  }
  return pts;
}
...
function buildNeonGlobe(): THREE.Group {
  ...
  const continents: [number,number][][] = [
    // North America
    [[72,-100],[68,-140],[60,-138],[55,-130],[50,-125],[44,-124],[38,-122],
     [32,-117],[28,-112],[24,-110],[20,-105],[16,-90],[10,-85],[8,-77],
     [10,-75],[18,-75],[25,-77],[30,-80],[35,-75],[40,-74],[44,-66],
     [47,-64],[52,-55],[57,-61],[62,-64],[68,-60],[72,-78],[72,-100]],
    // South America
    [[12,-72],[8,-62],[5,-52],[2,-50],[0,-50],[-5,-35],[-10,-37],
     [-15,-39],[-22,-43],[-28,-49],[-33,-52],[-38,-57],[-42,-63],
     [-50,-68],[-55,-68],[-48,-65],[-40,-62],[-33,-52],[-22,-43],
     [-10,-36],[-5,-35],[2,-50],[5,-52],[8,-62],[12,-72]],
    // Europe
    [[71,26],[68,18],[64,12],[58,5],[54,8],[50,2],[47,0],[45,3],[43,5],
     [40,0],[36,5],[36,14],[38,22],[40,28],[42,27],[45,30],[48,38],
     [54,38],[58,32],[62,28],[65,24],[68,18],[71,26]],
    // Africa
    [[37,9],[30,32],[22,37],[12,44],[0,42],[-10,40],[-18,35],[-26,32],
     [-34,25],[-34,18],[-28,17],[-22,14],[-15,12],[-5,9],[2,2],[5,-5],
     [10,-15],[15,-17],[20,-16],[25,-15],[28,-12],[32,-2],[34,6],[37,9]],
    // Asia
    [[70,30],[64,60],[68,100],[64,140],[57,140],[52,130],[45,135],[38,121],
     [30,122],[22,114],[10,104],[2,104],[-6,106],[2,100],[8,98],[12,80],
     [18,72],[22,68],[26,62],[22,56],[15,44],[12,44],[22,37],[30,32],
     [36,36],[40,42],[44,50],[46,60],[50,80],[55,70],[60,58],[64,58],[70,30]],
    // Australia
    [[-16,130],[-12,135],[-14,142],[-18,146],[-24,152],[-30,153],
     [-36,150],[-38,146],[-38,140],[-34,136],[-32,132],[-32,125],
     [-22,114],[-18,122],[-16,130]],
    // Greenland
    [[83,-35],[80,-58],[76,-68],[72,-72],[70,-52],[70,-30],[74,-18],[78,-18],[82,-25],[83,-35]],
  ];
  continents.forEach(c => drawContour(c));

  // City dots + pulse rings
  const cities: [number,number][] = [
    [40.71,-74.01],[51.51,-0.13],[48.85,2.35],[55.75,37.62],
    [35.69,139.69],[-33.87,151.21],[19.43,-99.13],[-23.55,-46.63],
    [28.61,77.21],[31.23,121.47],[1.35,103.82],[32.08,34.78],
    [25.2,55.27],[52.37,4.9],[41.9,12.5],
  ];
  ...
  cities.forEach(([lat, lon]) => {
    ...
    const pos = new THREE.Vector3(
      R * 1.010 * Math.sin(phi) * Math.cos(theta),
      R * 1.010 * Math.cos(phi),
      R * 1.010 * Math.sin(phi) * Math.sin(theta)
    );
  ...
  group.add(new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: 0x0066ff, transparent: true, opacity: 0.06, side: THREE.BackSide, depthWrite: false })
  ));
...
export default function OptimizedScene() {
  ...
  useEffect(() => {
    ...
    const renderer = new THREE.WebGLRenderer({ canvas, alpha: true, antialias: window.innerWidth > 1024, powerPreference: 'high-performance' });
    ...
    scene.add(new THREE.Points(starGeo, new THREE.PointsMaterial({ color: 0xaaccff, size: 0.08, transparent: true, opacity: 0.4 })));
    ...
    const onMouseDown  = (e: MouseEvent) => { isDragging = true; prevX = e.clientX; prevY = e.clientY; velocityX = velocityY = 0; };
    ...
    const animate = () => {
      ...
      camera.position.y += (mouseCurrent.y * (isMob ? 0.3 : 0.5) - scrollRatio * (isMob ? 1.0 : 1.8) - camera.position.y) * 0.06;
      ...
      if (modelRef.current) {
        ...
        if (rings) rings.forEach(({ mesh, phase }) => {
